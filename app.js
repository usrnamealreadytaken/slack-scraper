...
const token = 'xoxc-*************-*************-*************-????????????????????????????????????????';
const coookie = 'd=xoxd-??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????';
const channel = 'C0?????????';
const channelName='name';


const dataDir = 'data/'+channel+'_'+channelName
...
function getMessages(timestamp) {
  return new Promise(resolve => {
    https.get(
      `https://slack.com/api/conversations.history?token=${token}&channel=${channel}&pretty=1&latest=${timestamp || ''}`, 
        ...
        res.on('end', () => {
          try {
            ...
            if (parsedData.has_more) {
              ...
              fs.writeFile(`./${dataDir}/messages.json`, JSON.stringify(messages), (err) => {
                if (err) console.log(err);
...
function getUsers() {
  return new Promise(resolve => {
    https.get(`https://slack.com/api/users.list?token=${token}&pretty=1`,
      { headers: { Cookie: coookie } },
      (res) => {
        const {statusCode} = res;
        const contentType = res.headers['content-type'];
...
function setUsersAndMessages() {
  return new Promise(resolve => {
    fs.readFile(`./${dataDir}/messages.json`, 'utf-8', (err, data) => {
      if (err) {
        if (err.code === 'ENOENT') {
...
function createHTML(messages) {
  return new Promise(resolve => {
    ...
    const writeStream = fs.createWriteStream(`./${dataDir}/history.html`, { encoding: 'utf8' });
...
function resolveQuotes(data) {
  return new Promise(resolve => {
    if(data.message.attachments===undefined) return resolve(data);
    if(data.message.attachments[0].message_blocks===undefined) return resolve(data);
    // if(data.message.attachments[0].message_blocks.message===undefined) return resolve(data);
    // console.log(data.message.attachments[0].message_blocks[0].message.blocks[0].elements[0].elements[0].text)
    // if(){
    //   // console.log(data.message.attachments.message_blocks.message.blocks.elements.elements[1].text+ree);
    console.log(users[data.message.attachments[0].author_id]+data.message.attachments[0].author_id)
    data.text += 'quote from ' + 
      (users[data.message.attachments[0].author_id]!==undefined ?
        users[data.message.attachments[0].author_id].real_name:'Unknown User')
      + ':\n' + 
      (data.message.attachments[0].message_blocks[0].message.blocks[0].elements[0].elements[0] ?
        data.message.attachments[0].message_blocks[0].message.blocks[0].elements[0].elements[0].text:'') +
      (data.message.attachments[0].message_blocks[0].message.blocks[0].elements[0].elements[1] ?
        data.message.attachments[0].message_blocks[0].message.blocks[0].elements[0].elements[1].text:'')
    // }
    resolve(data)
  });
}
